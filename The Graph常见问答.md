...
The Graph是一个去中心化的协议，它能够对其他区块链项目的数据重新索引，以方便Dapp访问这些数据。现在支持以太坊，IPFS，未来会支持更多的区块链项目。The Graph让以前难以访问的数据变得简单直接，Dapp开发者不再需要关心如何取得自己需要的数据，可以非常轻松的从The Graph的数据库中获取。
...
如何联系项目方：
在Discord, 电报群组中均可联系到，或者发邮件到info@thegraph.com
...
我听说有一个Curator计划，是什么？
这是一个为想成为Curator角色而发起的激励计划。参与者通过完成一些特定的任务，可以领取GRT通证奖励。计划的注册已经关闭了。

GRT通证有什么用？
在The Graph的网络中，Indexer需要质押GRT通证来保证网络的安全，用户对数据的访问也需要支付GRT作为费用，Curator也会将GRT质押到优先级高的subgraph中。

我能买GRT通证吗？
通过预售买到的preGRT现在处于锁定状态。你需要等到主网上线，然后将preGRT转换成GRT。
